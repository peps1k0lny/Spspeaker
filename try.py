...
class Spspeaker:
    def __init__(self):
        ...
        self.volume = self.sp.current_playback()["device"]["volume_percent"] - self.sp.current_playback()["device"][
            "volume_percent"] % 5
        self.ismute = False
        ...
        try:
            while True:
                if self.ismute is False and GPIO.input(self.CLK_PIN) == GPIO.HIGH:
                    self.CLK_state = GPIO.input(self.CLK_PIN)
                    self.change_volume()
                if GPIO.input(self.SW_PIN) == GPIO.LOW:
                    self.mute(GPIO.input(self.SW_PIN))
    ...
    def change_volume(self):
        ...
        if self.CLK_state != self.prev_CLK_state and self.CLK_state == GPIO.HIGH:
            if GPIO.input(self.DT_PIN) == GPIO.HIGH and self.volume >= 0:
                self.volume -= 5
                self.direction = self.DIRECTION_CCW
            elif GPIO.input(self.DT_PIN) != GPIO.HIGH and self.volume <= 100:
                self.volume += 5
                self.direction = self.DIRECTION_CW

            print(self.volume)
            self.sp.volume(self.volume, device_id=self.DEVICE_ID)

        self.prev_CLK_state = self.CLK_state

    def mute(self, button_state):
        if button_state != self.prev_button_state:
            ...
            if self.ismute is True:
                self.volume = (self.sp.current_playback()["device"]["volume_percent"] -
